...
La técnica de \textit{AFM}, se clasifica en tres modos de operación, que son los siguientes:
 ...
 \begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{figures/chap3/beambounce.jpg}
        \caption{Diagrama esquemático de la punta del AFM y como se utiliza el laser para identificar la posición con un sensor\cite{Nanonics2}.}
    \label{fig:beambounce}
\end{figure}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\textwidth]{figures/chap3/nsom_sq.png}
        \caption{Diagrama esquemático del sistema NSOM\cite{Nanonics1}.}
    \label{fig:Nanonics}
\end{figure}

Una de las formas para sobrepasar este limite teórico, es utilizando la Microscopía Óptica de Campo Cercano (NSOM), donde se utiliza luz para observar los objetos, pero dentro del régimen de \textit{campo cercano}, donde es posible visualizar estructuras de tamaño comparable a la longitud de onda sin los problemas del límite de difracción, además que nos puede proporcionar propiedades interesantes del material ópticas y físicas.

Para censar, esta técnica utiliza una fuente de luz muy cercana a la superficie la cual tiene que estar en el redimen de campo cercano, y con el uso de piezoeléctricos realiza un barrido de ida y regreso (es configuración mas común). Usualmente esta técnica se realiza en paralelo con AFM.